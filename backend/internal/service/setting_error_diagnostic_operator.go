...
// 错误诊断的运维开关（管理员 API）。
//
// ADR 0005 要求生产采集与正文留存默认关闭，并由运维在完成**书面**风险确认后显式开启。
...
const (
	...
	ErrorDiagnosticRiskAcknowledgementVersion = "v2026.09.24"
...
//
// 只描述门控与确认记录本身：不含加密密钥、正文、账号或用户身份。三个布尔字段
// （Enabled／RiskAcknowledged／BodyRetentionEnabled）如实回显**存量门控值**，
// 而 CaptureAllowed／BodyRetentionAllowed 是**校验后**的结论：它们同时要求存量布尔值
// 与一条覆盖当前语句版本的有效书面确认（以及正文留存的可用密钥）。
//
...
//   - 正文留存是更窄的一层：需要开启 + 本次确认 + 可用密钥，三者缺一不可。
//   - 关闭永远允许，不需要确认或身份，并强制把正文留存一并关掉（兜底方向必须总能关）。
//
...
func (s *SettingService) UpdateErrorDiagnosticOperatorSettings(ctx context.Context, input ErrorDiagnosticOperatorUpdateInput) (ErrorDiagnosticOperatorStatus, error) {
	...
	// 关闭时这里是全 false：正文留存与风险确认位一并落回关闭。
