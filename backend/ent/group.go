...
type Group struct {
	...
	// thinking.type=disabled 时是否严格拒绝多余字段
	ThinkingDisabledStrict bool `json:"thinking_disabled_strict,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldThinkingDisabledStrict:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field thinking_disabled_strict", values[i])
			} else if value.Valid {
				_m.ThinkingDisabledStrict = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("thinking_disabled_strict=")
	builder.WriteString(fmt.Sprintf("%v", _m.ThinkingDisabledStrict))
	builder.WriteString(", ")
