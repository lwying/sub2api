...
// The UserVisibleAccountFunc type is an adapter to allow the use of ordinary
// function as UserVisibleAccount mutator.
type UserVisibleAccountFunc func(context.Context, *ent.UserVisibleAccountMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserVisibleAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserVisibleAccountMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserVisibleAccountMutation", m)
}
