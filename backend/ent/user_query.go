...
type UserQuery struct {
	...
	withVisibleAccounts       *AccountQuery
	...
	withUserVisibleAccounts   *UserVisibleAccountQuery
...
// QueryVisibleAccounts chains the current query on the "visible_accounts" edge.
func (_q *UserQuery) QueryVisibleAccounts() *AccountQuery {
...
// QueryUserVisibleAccounts chains the current query on the "user_visible_accounts" edge.
func (_q *UserQuery) QueryUserVisibleAccounts() *UserVisibleAccountQuery {
	query := (&UserVisibleAccountClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withVisibleAccounts:       _q.withVisibleAccounts.Clone(),
		...
		withUserVisibleAccounts:   _q.withUserVisibleAccounts.Clone(),
...
// WithVisibleAccounts tells the query-builder to eager-load the nodes that are connected to
// the "visible_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithVisibleAccounts(opts ...func(*AccountQuery)) *UserQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withVisibleAccounts = query
	return _q
}
...
// WithUserVisibleAccounts tells the query-builder to eager-load the nodes that are connected to
// the "user_visible_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithUserVisibleAccounts(opts ...func(*UserVisibleAccountQuery)) *UserQuery {
	query := (&UserVisibleAccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserVisibleAccounts = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [16]bool{
			...
			_q.withUserVisibleAccounts != nil,
		}
	)
	...
	if query := _q.withVisibleAccounts; query != nil {
		if err := _q.loadVisibleAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.VisibleAccounts = []*Account{} },
			func(n *User, e *Account) { n.Edges.VisibleAccounts = append(n.Edges.VisibleAccounts, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withUserVisibleAccounts; query != nil {
		if err := _q.loadUserVisibleAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.UserVisibleAccounts = []*UserVisibleAccount{} },
			func(n *User, e *UserVisibleAccount) {
				n.Edges.UserVisibleAccounts = append(n.Edges.UserVisibleAccounts, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadVisibleAccounts(ctx context.Context, query *AccountQuery, nodes []*User, init func(*User), assign func(*User, *Account)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*User)
	nids := make(map[int64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.VisibleAccountsTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(user.VisibleAccountsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.VisibleAccountsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.VisibleAccountsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "visible_accounts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadUserVisibleAccounts(ctx context.Context, query *UserVisibleAccountQuery, nodes []*User, init func(*User), assign func(*User, *UserVisibleAccount)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(uservisibleaccount.FieldUserID)
	}
	query.Where(predicate.UserVisibleAccount(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.UserVisibleAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
