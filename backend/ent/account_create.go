...
func (_c *AccountCreate) AddVisibleUserIDs(ids ...int64) *AccountCreate {
	_c.mutation.AddVisibleUserIDs(ids...)
	return _c
}

// AddVisibleUsers adds the "visible_users" edges to the User entity.
func (_c *AccountCreate) AddVisibleUsers(v ...*User) *AccountCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddVisibleUserIDs(ids...)
}
...
func (_c *AccountCreate) createSpec() (*Account, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.VisibleUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
