export default {
    settings: {
      ...
      requestAuditValueDetail: {
        title: 'Claude request-audit value details',
        description: 'A default-off, encrypted, 7-day bypass that retains allowlisted client and upstream header values, parsed metadata.user_id components and model names for Claude /v1/messages. The stored switch alone never captures anything.',
        hint: 'Capture runs only while the stored switch is on and an acknowledgement covering the current statement is on record. This is not a compliance deletion or data-subject erasure tool, and copies in replicas, backups, PITR and manual exports may persist.',
        loadFailed: 'Failed to load request-audit value-detail settings',
        unavailable: 'The value-detail gate could not be read. This is not a statement that capture is off; reload before changing anything.',
        saved: 'Request-audit value-detail settings updated',
        state: {
          storedLabel: 'Stored switch',
          on: 'On (stored)',
          off: 'Off (stored)',
          captureLabel: 'Effective capture',
          captureOn: 'Collecting',
          captureOff: 'Not collecting',
          keyLabel: 'Value encryption key',
          keyAvailable: 'Configured and restart-stable',
          keyUnavailable: 'Not configured or not restart-stable',
          captureMismatchStaleAck: 'The stored switch is on, but nothing is captured: the recorded acknowledgement does not cover the current statement. Acknowledging the current statement turns capture on.',
          captureMismatchNoAck: 'The stored switch is on, but nothing is captured: no written acknowledgement is in effect. Acknowledging the statement below turns capture on.',
          captureMismatchKey: 'The stored switch is on, but nothing is captured: no restart-stable encryption key is configured, so no value is retained.',
          keyRequired: 'Enabling is refused while no restart-stable encryption key is configured; configure a stable TOTP encryption key first.',
        },
        ack: {
          ...
          stale: 'The recorded acknowledgement does not cover the current statement version; it must be given again.',
        },
        enable: {
          title: 'Enable value details',
          notice: 'Enabling requires typing the current statement exactly. The statement is never stored in this browser, and the server re-verifies it on every update.',
          language: 'Statement language',
          requiredPhrase: 'Statement to type',
          phraseLabel: 'Type the statement to confirm',
          phrasePlaceholder: 'Type the statement above exactly',
          confirm: 'Acknowledge and enable',
          confirming: 'Enabling…',
          disabledHint: 'The confirmation stays unavailable until the statement is typed exactly.',
        },
        disable: {
          notice: 'Disabling needs no statement, no acknowledgement and no key, and always works.',
          action: 'Disable value details',
          disabling: 'Disabling…',
        },
        languages: {
          en: 'English',
        ...
        errors: {
          phraseRequired: 'The written acknowledgement statement is required to enable value details.',
          phraseInvalid: 'The statement does not match the current version verbatim; type it exactly as shown.',
          keyUnavailable: 'No configured, restart-stable encryption key is available, so value details cannot be enabled.',
          sessionRequired: 'Enabling requires an authenticated admin session, not an API key.',
          adminApiKeyForbidden: 'An admin API key can read and disable value details, but only an admin session can acknowledge and enable them.',
          rejected: 'The update was rejected. Nothing was changed.',
          forbidden: 'This admin identity may not enable value details.',
          unavailable: 'Value-detail settings are temporarily unavailable; nothing was changed.',
        },
      },
      webSearchEmulation: {
