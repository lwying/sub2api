export default {
    ...
    usage: {
      ...
      requestAudit: {
        ...
        wireRequestHeaders: '线上请求头',
        upstreamResponseHeaders: '上游响应头',
        upstreamStatus: '上游状态码',
        requestPayloadBytes: '请求载荷字节数',
        responsePayloadBytes: '响应载荷字节数',
        tokenCounts: {
          title: 'Token 用量',
          input: '输入 Token',
          output: '输出 Token',
        },
        responseReadComplete: '响应读取完整',
        protocolFields: {
          title: '协议字段',
          stream: '流式',
          thinkingType: 'Thinking 类型',
          presentFields: '已提供字段',
          normalizedFields: '规范化处理',
          thinkingDisabled: '禁用',
          thinkingEnabled: '启用',
          thinkingAdaptive: '自适应',
          normalizedThinkingExtraFieldsRemoved: '已移除 Thinking 多余字段',
          fieldModel: '模型',
          fieldMessages: '消息',
          fieldInput: '输入',
          fieldTools: '工具',
        ...
        clientResponse: {
          ...
          bytes: '写出到客户端字节数',
          observedNote:
            '处理器输出观测字节：这是处理器向客户端写出的字节，不代表客户端已收到，也不等于线上总字节数。永不保存响应正文。',
        },
        truncation: {
          originalEvents: '原始事件数',
          originalBytes: '原始字节数',
          keptEvents: '已记录事件数',
          keptBytes: '已记录字节数',
          droppedEvents: '丢弃事件数',
          droppedBytes: '丢弃字节数',
          reason: '原因',
          reasonMaxEvents: '达到最大事件数',
          reasonMaxBytes: '达到最大事件体量',
          reasonUnknown: '未知截断原因',
        },
        notCapturedReasons: {
          phase1Uncovered: '未采集：该入口不在第一阶段请求审计覆盖范围内。',
          unknown: '未采集：未提供覆盖原因。',
        },
        valueDetail: {
          title: '短期值明细',
          absent: '本次请求未采集短期值明细。',
          unavailable: '短期值明细读取失败。这不表示本次请求没有采集到值。',
          expiresAt: '值可读至',
          disabled: '本实例未开启短期值采集，因此不会留存这些值。',
          notice:
            '下方的头值、解析出的客户端标识与模型名仅在明确请求时才解密，只在数天内可读，且不会被缓存。此处绝不采集凭据、Cookie 与请求正文。',
          reveal: '查看短期值',
          revealing: '正在解密…',
          revealFailed: '无法获取短期值',
          truncated: '有部分提交的条目未被收下，因此此处可能未列出客户端发出的全部内容。',
          validationDropped: '有部分留存值未通过本视图的校验，因此此处可能未列出该记录中的全部内容。',
          model: '模型',
          inbound: '入站值',
          deviceId: '设备 ID',
          accountUuid: '账号 UUID',
          ...
          reasons: {
            notObserved: '本次请求未观察到可留存的短期值',
            retained: '短期值已留存',
            skippedOutOfScope: '本次请求未真正走 Anthropic 上游',
            skippedRetentionDisabled: '短期值留存已关闭',
            skippedEncryptionUnavailable: '加密不可用',
            skippedInvalidValues: '采集到的值未通过校验',
            skippedTooManyAttempts: '本次请求的上游尝试数超过可记录上限',
            unknown: '未知留存结果',
          },
        },
      },
