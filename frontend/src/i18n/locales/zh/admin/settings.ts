export default {
    settings: {
      ...
      requestAuditValueDetail: {
        title: 'Claude 请求审计值明细',
        description: '默认关闭、加密保留 7 天的旁路，仅针对 Claude /v1/messages，留存白名单内的客户端与上游头值、解析出的 metadata.user_id 组件与模型名。仅存量开关为真时不会采集任何内容。',
        hint: '只有存量开关为真、且存在覆盖当前语句版本的书面确认时才允许采集。本功能不是合规删除或数据主体擦除手段；只读副本、备份、PITR 与人工导出中的旧数据可能仍留存。',
        loadFailed: '加载请求审计值明细设置失败',
        unavailable: '值明细门控读取失败。这不等于「采集已关闭」；请在改动前重新加载。',
        saved: '请求审计值明细设置已更新',
        state: {
          storedLabel: '存量开关',
          on: '开（存量）',
          ...
          captureOff: '未采集',
          keyLabel: '值加密密钥',
          keyAvailable: '已配置且重启后不变',
          keyUnavailable: '未配置或重启后会变化',
          captureMismatchStaleAck: '存量开关为开，但实际没有采集：在库的书面确认不覆盖当前语句。按当前语句重新确认后即开始采集。',
          captureMismatchNoAck: '存量开关为开，但实际没有采集：没有生效的书面确认。逐字确认下方语句后即开始采集。',
          captureMismatchKey: '存量开关为开，但实际没有采集：没有配置重启后不变的加密密钥，任何值都不会留存。',
          keyRequired: '未配置重启后不变的加密密钥时，开启会被拒绝；请先配置稳定的 TOTP 加密密钥。',
        },
        ack: {
          version: '语句版本',
          operator: '确认管理员',
          ...
          phrase: '确认的语句',
          none: '尚未记录书面风险确认。',
          stale: '在库的确认不覆盖当前语句版本，需要重新逐字确认。',
        },
        enable: {
          title: '开启值明细',
          notice: '开启必须逐字输入当前语句。语句不会保存在本浏览器中，且服务端每次更新都会重新校验。',
          language: '语句语言',
          requiredPhrase: '需要输入的语句',
          phraseLabel: '输入语句以确认',
          phrasePlaceholder: '逐字输入上方语句',
          confirm: '确认并开启',
          confirming: '开启中…',
          disabledHint: '只有逐字输入语句后，确认按钮才可用。',
        },
        disable: {
          notice: '关闭不需要语句、不需要确认记录、也不需要密钥，任何时候都可以执行。',
          action: '关闭值明细',
          disabling: '关闭中…',
        },
        languages: {
        ...
        errors: {
          phraseRequired: '开启值明细必须提交书面确认语句。',
          phraseInvalid: '语句与当前版本不一致；请照原文逐字输入。',
          keyUnavailable: '没有可用的、重启后不变的加密密钥，无法开启值明细。',
          sessionRequired: '开启需要已登录的管理员会话，不接受 API Key。',
          adminApiKeyForbidden: '管理 API Key 可以读取和关闭值明细，但只有管理员会话才能确认并开启。',
          rejected: '更新被拒绝，未做任何改动。',
          forbidden: '当前管理员身份无权开启值明细。',
          unavailable: '值明细设置暂时不可用，未做任何改动。',
        },
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
