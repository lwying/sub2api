...
// 只读账号视图入口只对获得管理员授权的普通用户显示。
// 它来自当前用户资料（60 秒轮询刷新），仅决定菜单是否出现；
// 路由守卫会在进入页面前实时复核，管理员继续使用 /admin/accounts。
const flagAssignedAccounts = () => authStore.canViewAssignedAccounts
...
//
// 条目顺序：密钥 → 账号 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/accounts', label: t('nav.assignedAccounts'), icon: GlobeIcon, featureFlag: flagAssignedAccounts },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/error-diagnostics', label: t('admin.errorDiagnostics.title'), icon: ShieldIcon },
